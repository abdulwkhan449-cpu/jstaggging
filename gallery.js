 // Copy all your JavaScript code from the original header's <script> block
      ...
      (function () {
            ...
            var closeBtn = document.getElementById('close-lightbox-btn');
            var prevBtn = document.getElementById('prev-btn');
            var nextBtn = document.getElementById('next-btn');
            var grid = document.getElementById('gallery-grid');

            var currentIndex = 0;
            var total = imagesData.length;

            function openLightbox(index) {
                if (!modal || !imagesData.length) return;
                if (index < 0) index = 0;
                if (index >= total) index = total - 1;
                currentIndex = index;
                var img = imagesData[currentIndex];
            ...
            function nextImage(e) { if (e) e.stopPropagation(); if (currentIndex < total - 1) openLightbox(currentIndex + 1); }
            function prevImage(e) { if (e) e.stopPropagation(); if (currentIndex > 0) openLightbox(currentIndex - 1); }
            ...
            if (grid) {
                grid.addEventListener('click', function (e) {
                    ...
                    if (!isNaN(index) && imagesData[index]) openLightbox(index);
                });
            }
        ...
        // ============================================
        // BEFORE/AFTER SLIDER HANDLES (ALL 3)
        // ============================================
        // ==========================================================
        // SMOOTH BEFORE/AFTER SLIDER HANDLES (ALL 3)
        // Uses requestAnimationFrame for silky performance
        // ==========================================================
        (function () {
            // ---- Define your three sliders ----
            const sliders = [
                { container: 'slider1', clip: 'after-clip1', handle: 'handle1' },
                { container: 'slider2', clip: 'after-clip2', handle: 'handle2' },
                { container: 'slider3', clip: 'after-clip3', handle: 'handle3' }
            ];

            // ---- Loop through each and apply the same logic ----
            sliders.forEach(function (config) {
                const container = document.getElementById(config.container);
                const afterClip = document.getElementById(config.clip);
                const handle = document.getElementById(config.handle);

                if (!container || !afterClip || !handle) {
                    console.warn('Slider not found:', config.container);
                    return;
                }
                ...
                // ---- Core update (applies the position) ----
                function updateSlider(clientX) {
                    const rect = container.getBoundingClientRect();
                ...
                function updateSliderSmooth(clientX) {
                    pendingX = clientX;
                    if (rafId) return;
                    rafId = requestAnimationFrame(function () {
                        if (pendingX !== null) {
                            updateSlider(pendingX);
                            pendingX = null;
                        }
                        rafId = null;
                    });
                ...
                function disableTransitions() {
                    afterClip.style.transition = 'none';
                ...
                container.addEventListener('touchstart', function (e) {
                    ...
                    if (touch) {
                        isDragging = true;
                        disableTransitions();
                        updateSliderSmooth(touch.clientX);
                    }
                }, { passive: true });
                ...
                // ========== CLICK TO JUMP (with smooth transition) ==========
                container.addEventListener('click', function (e) {
                    if (e.target.closest('#' + config.handle)) return;
                    if (!isDragging) {
                        // Animate the jump
                        afterClip.style.transition = 'width 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94)';
                        handle.style.transition = 'left 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94)';
                        updateSlider(e.clientX);
                        setTimeout(function () {
                            afterClip.style.transition = '';
                            handle.style.transition = '';
